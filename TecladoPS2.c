...
{											// entonces hago la conversión.
	...
	if(lect_ps2==0x66||(lect_ps2==0x71&&ctrl_ps2))	// Se presionó DEL O DELETE???
	{
		...
		teclado_str[pos_str_ps2]=0;			// Borro el contenido del lugar.
	...
	if(lect_ps2==0x76)						// Presionaron la tecla ESC borro la linea.
	{
		...
		teclado_str[0]=0;					// Marco el final de la cadena.
		return;								// Salgo a la interrupción.
	}
	...
	if(lect_ps2==0x5a||(lect_ps2==0x5a&&ctrl_ps2))	// Si es un final de linea proceso el comando.
	{
		teclado_str[pos_str_ps2]=0;			// Marco el final de la cadena.
		return;								// Salgo a la interrupción.
	}
	cont_ps2=0;
	do										// Recorro la tabla en busca del ASCII.
	{
		...
		if(lect_ps2==ps2[cont_ps2][0])		// Comparo el codigo leido con la tabla.
		{
			...
			{
				teclado_str[pos_str_ps2]=ps2[cont_ps2][1];
				break;
			}
			if(mayus_ps2&&!shift_ps2)		// Estaba presionada la tecla SHIFT???
			{
				teclado_str[pos_str_ps2]=ps2[cont_ps2][2];
				break;
			}
			if(!mayus_ps2&&shift_ps2)		// Estaba presionada la tecla MAYUSCULA y SHIFT???
			{
				teclado_str[pos_str_ps2]=ps2[cont_ps2][3];
				break;
			}
			if(mayus_ps2&&shift_ps2)		// Estaba presionada la tecla MAYUSCULA y SHIFT???
				teclado_str[pos_str_ps2]=ps2[cont_ps2][4];
			break;							// Una vez encontrado salgo del bucle.
		}
	}while(++cont_ps2<64);					// Busco hasta el último caracter de la tabla.
	pos_str_ps2++;
	if(pos_str_ps2>14)						// Limito a 16 caracteres el comando.
		pos_str_ps2=14;						// Voy al siguiente lugar en la cadena.
	teclado_str[pos_str_ps2]=0;				// Marco el final de la cadena
	return;									// Salgo a la interrupción.
}
/****************************************************************************************/
...
{
	CLRWDT();
	teclado_str[0]=0;	// Reseteo la cadena.
	...
	INTF=0;				// Bajo la bandera de la interrupcion.
	INTE=1;				// Habilito la interrupción del teclado.
	return;
}
